...
import sys
import csv
print("Fast Start time: ", datetime.datetime.now())
...
crossing_list.drop("Inst", axis = 1, inplace = True)
...
#converting crossing types to numerical values
crossing_dict = {"MPI":1, "MPO":2, "BSI":3, "BSO":4}
crossing_list.replace({"Type": crossing_dict}, inplace = True)

df["Date"] = pd.to_datetime(df["Date"], format = "%Y-%m-%d %H:%M:%S")


#Initial column for transitions (to be changed later)
df["Transition"] = "No Transition"
...
df.loc[df["Transition"] == "No Transition"].to_csv("validation_list_no_transitions_fast.csv")
del df
print("Fast End time: ", datetime.datetime.now())
